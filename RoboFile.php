...
class RoboFile extends \Robo\Tasks
{
     ...
     *
     */
    const REPO_BLOB_URL = 'https://github.com/pantheon-systems/terminus-api/blob';
    ...
    public function release() : void
    {
        ...
        $this->say('TERMINUS RELEASE: ' . $version);
        $this->update();
        $this->buildDocs();
        $this->publishDocs();
        $this->buildPhar();
    ...
    public function getCache() {
        ...
        return \Http\Client\Common\Plugin\CachePlugin::clientCache(
            $pool,
            $streamFactory,
            []
        );
    ...
    {
        return $this->taskExec("git rev-parse --abbrev-ref HEAD");
    }
     ...
     *
     */
    public function buildCommandDocs()
    {
        $this->taskCleanDir('docs/modules')->run();
        $this->say("Modules");
        $commands = Finder::create()->files()->name('*.php')->in(__DIR__ . '/src/Commands');

        foreach ($commands as $command) {
            $commandName = basename(substr($command, 0, -4));
            $className = 'Pantheon\Terminus\API\Commands\\' . $commandName;
            $source = "https://github.com/pantheon-systems/terminus-api/tree/"
                . $this->branch() . "/src/Codeception/Module/$commandName.php";

            $this->taskGenDoc('docs/modules/' . $commandName . '.md')
                ->docClass($className)
                ->prepend('# ' . $commandName)
    ...
    protected function processChangelog()
    {
        $sortByVersionDesc = function (\SplFileInfo $a, \SplFileInfo $b) {
            ...
            if (preg_match($pattern, $a->getBasename(), $matches1) && preg_match(
                $pattern,
                $b->getBasename(),
                $matches2
            )) {
    ...
    public function publishPhar()
    {
        ...
        foreach ($releases as $release) {
            ...
            [$major, $minor] = explode('.', $releaseName);
